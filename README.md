...
### 📦 `out.sh` - Script de déménagement (sauvegarde/nettoyage)

Prépare votre machine avant un changement d'ordinateur :

```bash
./out.sh                   # Menu interactif
./out.sh --sauvegarde      # Sauvegarde uniquement
./out.sh --nettoyage       # Nettoyage uniquement
./out.sh --complet         # Sauvegarde + nettoyage
```

**Fonctionnalités :**
- Sauvegarde de la configuration Homebrew
- Sauvegarde des dotfiles (.zshrc, .gitconfig, etc.)
- Export des extensions VS Code
- Sauvegarde des paramètres système
- Nettoyage sélectif des applications

### ⚙️ `brew-manager.sh` - Gestion avancée Homebrew

Script de gestion fine des packages Homebrew :

```bash
./brew-manager.sh install     # Installer depuis Brewfile
./brew-manager.sh uninstall   # Désinstaller tout
./brew-manager.sh update      # Mettre à jour
./brew-manager.sh list        # Lister les packages
./brew-manager.sh backup      # Sauvegarder la config
```
...
├── in.sh              # Script d'emménagement
├── out.sh             # Script de déménagement
├── brew-manager.sh    # Gestion avancée Homebrew
├── utils.sh           # Fonctions communes (couleurs, etc.)
├── Brewfile           # Configuration des packages de base
├── Brewfile.minimal   # Configuration minimale
├── Brewfile.complet   # Configuration complète
├── Brewfile.dev       # Configuration développeur
├── Brewfile.designer  # Configuration designer
└── README.md          # Ce fichier
```

## 🔄 Workflow recommandé

### Nouveau Mac (emménagement)

1. **Cloner ce dépôt** :
   ```bash
   git clone <url-du-depot> ~/setup
   cd ~/setup
   ```

2. **Emménager rapidement** :
   ```bash
   ./setup.sh in
   ```

3. **Ou choisir un profil spécifique** :
   ```bash
   ./in.sh --dev        # Pour développeur
   ./in.sh --designer   # Pour designer
   ./in.sh --minimal    # Installation légère
   ```

### Ancien Mac (déménagement)

1. **Sauvegarder votre configuration** :
   ```bash
   ./setup.sh out --sauvegarde
   ```

2. **Transférer les sauvegardes** vers le nouveau Mac

3. **Nettoyer avant de vendre/donner** :
   ```bash
   ./out.sh --nettoyage
   ```
...
./brew-manager.sh update

# Sauvegarder la configuration actuelle
./brew-manager.sh backup
```

## ✨ Fonctionnalités

- ✅ **Installation automatique** de Homebrew
- ✅ **Profils prédéfinis** (minimal, complet, dev, designer)
- ✅ **Sauvegarde complète** avant déménagement
- ✅ **Interface colorée** et intuitive
- ✅ **Gestion d'erreurs** robuste
- ✅ **Compatible** macOS Intel et Apple Silicon
- ✅ **Scripts modulaires** et réutilisables

## 🛠️ Personnalisation

### Ajouter des outils au Brewfile

```bash
# Éditez le Brewfile principal
vim Brewfile

# Ou créez votre propre profil
cp Brewfile Brewfile.monprofil
```

### Modifier les sauvegardes

Éditez `out.sh` pour ajouter vos propres fichiers à sauvegarder.

## 🚨 Notes importantes

- Les scripts demandent confirmation avant les actions destructives
- Les sauvegardes sont horodatées automatiquement
- Le nettoyage ne supprime que les packages gérés par ces scripts
- Testez d'abord sur un environnement non critique

## 🤝 Utilisation

1. **Première utilisation** : `./setup.sh in`
2. **Changement de machine** : `./setup.sh out` puis `./setup.sh in` sur le nouveau Mac
3. **Maintenance** : `./brew-manager.sh update`

---

**💡 Conseil** : Commitez vos modifications de Brewfile dans votre fork pour garder votre configuration personnalisée !